...
export function updateUserDisplay(status) {
  ...
  if (status === 'searching') {
    ...
    if (state.currentUserVipConfig) {
        ...
        userInfoSpan.textContent = `${state.currentUserVipConfig.name} (VIP, L${state.userLevel})`;
    } else {
...
export function searchProducts() {
  ...
  if (!hasKey && !hasQty && !hasBudget && !hasCat && !hasBrand) {
    alert("請輸入搜尋條件（關鍵字、品牌、分類、起訂量或預算）");
    return;
  }
